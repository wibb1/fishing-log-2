...
#run this code to change the success column from string to enum (only needs to be done once)
Records.all.each do |entry|
  entry.success = %w(Good, Bad, OK).index(entry.old_success)
  entry.save!
end